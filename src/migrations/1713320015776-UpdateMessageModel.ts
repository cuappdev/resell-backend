...
export class UpdateMessageModel1713320015776 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "Message" ADD "content" character varying NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "Message" ADD "receiver" uuid NOT NULL`,
    );
    await queryRunner.query(`ALTER TABLE "Message" ADD "sender" uuid NOT NULL`);
    await queryRunner.query(
      `ALTER TABLE "Message" ADD "timestamp" TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()`,
    );
    await queryRunner.query(
