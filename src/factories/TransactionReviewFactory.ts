...
define(
  ...
  (_, context?: { index?: number; transaction?: TransactionModel }) => {
    if (
      context === undefined ||
    ...
    transactionReview.comments =
      index % 2 === 0 ? `Great transaction! - Index: ${index}` : null; // Alternate between having comments or not
    transactionReview.hadIssues = index % 3 === 0; // Alternate having issues
    transactionReview.issueCategory = transactionReview.hadIssues
      ? `Issue Category - Index: ${index}`
      : null; // Issue category if hadIssues is true
    transactionReview.issueDetails = transactionReview.hadIssues
      ? `Detailed issue description for transaction review - Index: ${index}`
      : null; // Issue details if hadIssues is true
    transactionReview.createdAt = new Date(
