...
export class TransactionReviewService {
  ...
  public async getTransactionReviewById(
    ...
    return this.transactions.readOnly(async (transactionalEntityManager) => {
      ...
      const transactionReview =
        await transactionReviewRepository.getTransactionReviewById(params.id);
      if (!transactionReview)
        throw new NotFoundError("Transaction Review not found!");
  ...
  public async getTransactionReviewByTransactionId(
    ...
    return this.transactions.readOnly(async (transactionalEntityManager) => {
      ...
      const transactionReview =
        await transactionReviewRepository.getTransactionReviewByTransactionId(
          transactionId.id,
        );
      if (!transactionReview)
        throw new NotFoundError(
          "Transaction Review for the given transaction not found!",
        );
      return transactionReview;
    });
  }
  ...
  public async createTransactionReview(
    review: CreateTransactionReviewRequest,
  ): Promise<TransactionReviewModel> {
    return this.transactions.readWrite(async (transactionalEntityManager) => {
      ...
      const freshTransactionReview =
        await transactionReviewRepository.createTransactionReview(
          review.stars,
  ...
  public async deleteTransactionReviewById(
    params: UuidParam,
  ): Promise<TransactionReviewModel> {
    return this.transactions.readWrite(async (transactionalEntityManager) => {
      const transactionReviewRepository = Repositories.transactionReview(
      ...
      if (!transactionReview)
        throw new NotFoundError("Transaction Review not found!");
      return transactionReviewRepository.deleteTransactionReview(
        transactionReview,
      );
    });
  }
