...
export class UserReviewService {
  ...
  public async getUserReviewById(params: UuidParam): Promise<UserReviewModel> {
    return this.transactions.readOnly(async (transactionalEntityManager) => {
      ...
      const userReview = await userReviewRepository.getUserReviewById(
        params.id,
      );
  ...
  public async createUserReview(
    userReview: CreateUserReviewRequest,
    ...
    return this.transactions.readWrite(async (transactionalEntityManager) => {
      ...
      const freshUserReview = await userReviewRepository.createUserReview(
        userReview.fulfilled,
        userReview.stars,
        userReview.comments,
        buyer,
        seller,
  ...
  public async deleteUserReviewById(
    buyer: UserModel,
    params: UuidParam,
  ): Promise<UserReviewModel> {
    return this.transactions.readWrite(async (transactionalEntityManager) => {
      ...
      if (buyer.firebaseUid != userReview.buyer?.firebaseUid && !buyer.admin)
        throw new ForbiddenError("User is not buyer!");
