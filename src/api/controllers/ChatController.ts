...
export class ChatController {
  ...
  async postChat(
    ...
    @Params() params: ChatParam,
    @Body() chatBody: CreateChatMessage,
  ...
  async postAvailability(
    @CurrentUser() user: UserModel,
    @Params() params: ChatParam,
    @Body() chatBody: CreateAvailabilityChat,
  ): Promise<AvailabilityResponse> {
    const chatId = params.id;
    const doc = await chatsRef.doc(chatId).get();
    const now = new Date();

    const processedAvailabilities = chatBody.availabilities.map(
      (availability) => ({
  ...
  async sendProposal(
    @CurrentUser() user: UserModel,
    @Params() params: ChatParam,
    @Body() chatBody: CreateProposalChat,
  ...
  async respondProposal(
    @CurrentUser() user: UserModel,
    @Params() params: ChatParam,
    @Body() chatBody: RespondProposalChat,
  ): Promise<ProposalResponse> {
    const chatId = params.id;
    const doc = await chatsRef.doc(chatId).get();
    const now = new Date();
