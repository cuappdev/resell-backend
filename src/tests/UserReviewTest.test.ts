...
describe("user review tests", () => {
  ...
  test("get user review by id", async () => {
    ...
    const getUserReviewResponse =
      await userReviewController.getUserReviewsById(uuidParam);
    getUserReviewResponse.userReview.stars = Number(
      getUserReviewResponse.userReview.stars,
    );
  ...
  test("create user review", async () => {
    ...
    await new DataFactory().createUsers(buyer, seller).write();
    ...
    const getUserReviewResponse =
      await userReviewController.createUserReview(newUserReview);
    const getUserReviewsResponse = await userReviewController.getUserReviews();

    expect(getUserReviewResponse.userReview.comments).toEqual(
