...
describe("user tests", () => {
  ...
  test("get all users - multiple users", async () => {
    ...
    await new DataFactory().createUsers(user1, user2).write();
  ...
  test("get user by email", async () => {
    ...
    const getUserResponse = await userController.getUserByEmail({
      email: "sn999@cornell.edu",
    });
    if (getUserResponse.user != undefined) {
      getUserResponse.user.stars = Number(getUserResponse.user.stars);
    }
  ...
  test("get user by google id", async () => {
    ...
    const getUserResponse =
      await userController.getUserByGoogleId("shungoGoogleID");
    if (getUserResponse.user != undefined) {
      getUserResponse.user.stars = Number(getUserResponse.user.stars);
    }
    expect(omit(getUserResponse.user, "id")).toEqual(omit(expectedUser, "id"));
  ...
  test("set admin status from super user", async () => {
    ...
    await new DataFactory().createUsers(superAdmin, regularUser).write();
  ...
  test("block users", async () => {
    ...
    const blockUserResponse = await userController.blockUser(
      { blocked: user2.firebaseUid },
      user1,
    );
  ...
  test("unblock users", async () => {
    ...
    const unblockUserResponse = await userController.unblockUser(
      { unblocked: user2.firebaseUid },
      user1,
    );
  ...
  test("not returning posts when one user blocks another (having a neutral user)", async () => {
    ...
    expect(new Set(blockerSavedPosts.posts.map((post) => post.id))).toEqual(
      new Set([blockedPost.id, neutralPost.id]),
    );
    ...
    expect(new Set(blockedSavedPosts.posts.map((post) => post.id))).toEqual(
      new Set([blockerPost.id, neutralPost.id]),
    );
    ...
    expect(new Set(neutralSavedPosts.posts.map((post) => post.id))).toEqual(
      new Set([blockedPost.id, blockerPost.id]),
    );
    ...
    expect(blockerSavedPosts).not.toBeUndefined();
    expect(blockerSavedPosts.posts.map((post) => post.id)).toEqual([
      neutralPost.id,
    ]);
    ...
    expect(blockedSavedPosts).not.toBeUndefined();
    expect(blockedSavedPosts.posts.map((post) => post.id)).toEqual([
      neutralPost.id,
    ]);

    // Blocker unblocks blocked
    await userController.unblockUser(
      { unblocked: blocked.firebaseUid },
      blocker,
    );
    // Verify blocker has saved posts
    blockerSavedPosts = await postController.getSavedPostsByUserId(blocker);
  ...
  test("delete users - user deletes themselves", async () => {
    ...
    const deleteUserResponse = await userController.deleteUserByOtherUser(
      uuidParam,
      user,
    );
  ...
  test("delete users - user deletes themselves with fcm", async () => {
    ...
    await new DataFactory().createUsers(user).createFcmTokens(fcmToken).write();
  ...
  test("delete users - user deletes another user", async () => {
    ...
    await new DataFactory().createUsers(admin, user).write();
  ...
  test("delete users - user that is not an admin tries to delete another user", async () => {
    ...
    try {
      await userController.deleteUserByOtherUser(
        { id: user2.firebaseUid },
        user1,
      );
    } catch (error) {
  ...
  test("get blocked users by id - no blocked users", async () => {
    ...
    const getBlockedUsersResponse =
      await userController.getBlockedUsersById(userUuid);
  ...
  test("get blocked users by id", async () => {
    ...
    const getBlockedUsersResponse =
      await userController.getBlockedUsersById(user1Uuid);
  ...
  test("soft delete, get all users with some users active, some inactive", async () => {
    ...
    await new DataFactory().createUsers(user1, user2, user3).write();
